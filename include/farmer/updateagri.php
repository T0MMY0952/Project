...
if(empty($ap_name) || empty($ap_collectdate)  || empty($ap_amount) || empty($ap_unit) || empty($ap_exportdate) || empty($idsendto)){
	echo '<script type="text/javascript" >
		...
		if(substr($idsendto, 0, 1) == 's'){
			$sql2 = "UPDATE shipment SET idseller_recieve = '".substr($idsendto, 1)."', idfactory_recieve = NULL WHERE idshipment = $idapr ";
			$result2 = $con->Query($sql2) or die(mysqli_error($con));
			
		}else{
			$sql2 = "UPDATE shipment SET idfactory_recieve = $idsendto, idseller_recieve = NULL WHERE idshipment = $idapr ";
			$result2 = $con->Query($sql2) or die(mysqli_error($con));
			
		}
		
		$sql1 = "UPDATE agriculture_product as ap  SET ap_name = '$ap_name', ap_collectdate = '$ap_collectdate' , ap_garden = '$ap_garden' , ap_amount = '$ap_amount', ap_unit = '$ap_unit', ap_exportdate = '$ap_exportdate' , ap_price = '$ap_price' WHERE idagriculture_product = (SELECT idagriculture_product FROM shipment WHERE idshipment = $idapr) " ;
