	...
	if (empty($email) || empty($password) || empty($repassword) || empty($name) || empty($surname)  || empty($businessname) || empty($businessaddress) || !is_numeric($type) || 
		...
		$row=$result->fetch_assoc();
			...
			echo '<script type="text/javascript" >
				...
				if ($type == 1) {
					//--farmer regis
					$insertuser = $con->query("INSERT INTO user_account (iduser_account,email,pass,type,status,lastupdate) VALUES ('','$email','$password','farmer','','')");
					//--Check farmname is duplicate
					$duplicate = $con->query("SELECT COUNT(*) AS count FROM farm_place WHERE farmname = '$businessname'");
					...
					if($row1['count']){	
						...
						(SELECT idfarm_place FROM farm_place WHERE farm_place.farmname = '$businessname'), (SELECT iduser_account FROM user_account WHERE user_account.email = '$email'),'$name','$surname','$tel','$peoplepic')");
					}else{
					
						$insertfarm = $con->query("INSERT INTO farm_place (idfarm_place,farmname,farmaddress,farmtel,farmpic) VALUES ('','$businessname','$businessaddress','$businesstel','$placepic')");
						$insertfarmer = $con->query("INSERT INTO farmer(idfarm_place,iduser_account,farmername,farmersurname,farmertel,farmerpic) VALUES(
						(SELECT idfarm_place FROM farm_place WHERE farm_place.farmname = '$businessname'), (SELECT iduser_account FROM user_account WHERE user_account.email = '$email'),'$name','$surname','$tel','$peoplepic')");
					}
					echo '<script type="text/javascript" >
				...
				if ($type == 2) {
					//--factory regis
					$insertuser = $con->query("INSERT INTO user_account (iduser_account,email,pass,type,status,lastupdate) VALUES ('','$email','$password','factoryadmin','','')");
					...
					if($checkdup = $duplicate->fetch_assoc()){
						echo '<script type="text/javascript" >
					...
					if($row1['count']){
						...
						'$name','$surname','$tel','$peoplepic')");
					}else{
					$insertfactory = $con->query("INSERT INTO factory_place (idfactory_place,factoryname,factoryaddress,factorytel,factorypic) VALUES ('','$businessname','$businessaddress','$businesstel','$placepic')");
					$insertfactory_staff = $con->query("INSERT INTO factory_staff(idfactory_place,iduser_account,factory_staffname,factory_staffsurname,factory_stafftel,factory_staffpic) VALUES(
						(SELECT idfactory_place FROM factory_place WHERE factory_place.factoryname = '$businessname'), (SELECT iduser_account FROM user_account WHERE user_account.email = '$email'),
						'$name','$surname','$tel','$peoplepic')");
					}
				...
				if ($type == 3) {
					//--seller regis
					$insertuser = $con->query("INSERT INTO user_account (iduser_account,email,pass,type,status,lastupdate) VALUES ('','$email','$password','selleradmin','','')");
					...
					if($row1['count']){
						...
						(SELECT idseller_place FROM seller_place WHERE seller_place.sellername = '$businessname'), (SELECT iduser_account FROM user_account WHERE user_account.email = '$email'),'$name','$surname','$tel','$peoplepic')");
					}else{
